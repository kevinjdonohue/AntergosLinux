...
UPDATE: As of June 20th, 2018 I removed both `pacaur` and `pacget` as I started running into issues updating AUR packages.

As an alternative, since pacaur (and by reference pacman) are no longer being developed or supported, I've migrated to another AUR package management tool called `aurman`.

As mentioned in their documentation, their syntax is _almost_ the same as `pacman`.

### [yaourt](https://archlinux.fr/yaourt-en)

UPDATE: As of June 20th, 2018 I've started using `aurman` instead of `pacaur` and `pacget`. In addition, since `yaourt` is also listed as an unsupported package manager I've migrated (back) to `pacman` for regular package management and `aurman` for AUR package management.
